...
int main(int argc, char const *argv[]) {
  ...
  cin >> start_year;
  int start_number_of_days_since_epoch{start_day - 32075 + 1461
  ...
  cin >> end_year;
  int end_number_of_days_since_epoch{end_day - 32075 + 1461
    * (end_year + 4800 + (end_month - 14) / 12) / 4
    + 367 * (end_month - 2 - (end_month - 14) / 12 * 12) / 12 - 3
    * ((end_year + 4900 + (end_month - 14) / 12) / 100) / 4};

  int number_days{end_number_of_days_since_epoch - start_number_of_days_since_epoch};
  cout << "The number of days between " << start_month << "/" << start_day << "/" << start_year
       << " and " << end_month << "/" << end_day << "/" << end_year << " is " << number_days 
       << " days\n";
  return 0;
}